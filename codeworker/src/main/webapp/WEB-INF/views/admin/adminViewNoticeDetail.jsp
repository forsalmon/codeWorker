							...
							<div class="card-body">
								<div class="row gy-3">
									<div class="col-xl-6">
										...
										<input type="text" class="form-control" name="boardTitle" id="boardTitle" value="${noticeDetail.boardTitle }" readonly="readonly">
									</div>
									<div class="col-xl-6">
										<label for="blog-last-name" class="form-label">등록일</label> 
										<input type="text" class="form-control" name="boardRegDate" id="boardRegDate" value="${noticeDetail.boardRegDate }" readonly="readonly">
									</div>
									<div class="col-xl-6">
									    <label for="blog-email" class="form-label">분류</label> 
									    <input type="text" class="form-control" name="boardTypeCode" id="boardTypeCode" readonly="readonly" value="<c:if test="${noticeDetail.boardTypeCode eq 'BOT0101'}">공지사항</c:if><c:if test="${noticeDetail.boardTypeCode eq 'BOT0102'}">FAQ</c:if>">
									</div>
									<div class="col-xl-6">
										<label for="blog-email" class="form-label">타겟</label> 
										<input type="text" class="form-control" name="boardTargetCode" id="boardTargetCode" readonly="readonly" value="<c:if test="${noticeDetail.boardTargetCode eq 'TAR0101'}">기업회원</c:if><c:if test="${noticeDetail.boardTargetCode eq 'TAR0102'}">구직회원</c:if>">
									</div>
									<div class="col-xl-12">
										<label for="blog-comment" class="form-label">내용</label>
										<textarea class="form-control" name="boardContent" id="boardContent" rows="5" readonly="readonly">${noticeDetail.boardContent }</textarea>
									</div>
								</div>
							</div>
							<div class="card-footer">
								<div class="text-end">
									<button class="btn btn-primary-light" onclick="javascript:location.href='/admin/noticeList'">목록</button>
            ...
            if (confirm('삭제하시겠습니까?')) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response === 'success') {
                            alert('삭제되었습니다.');
                            location.href = '/admin/noticeList'; // 목록 페이지로 이동
                        } else {
                            alert('삭제에 실패했습니다.');
                        }
                    },
