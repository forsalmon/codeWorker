...
<%--                         	<c:set value="${pagingVO.dataList }" var="noticeList"/> --%>
                            <c:forEach items="${noticeList}" var="notiList" varStatus="status">
                                <tr class="${notiList.delYn eq 'Y' ? 'deleted' : ''}" data-target-code="${notiList.boardTargetCode}">
                                	<td>${pagingVO.startRow + status.index}</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${notiList.boardTargetCode eq 'TAR0101' }">
												기업회원
                                            </c:when>
                                            <c:when test="${notiList.boardTargetCode eq 'TAR0102' }">
												구직회원
                                            </c:when>
                                        </c:choose>
                                    </td>
                                    <td>
                                        <a href="/admin/noticeDetail?boardNo=${notiList.boardNo }" style="color: ${notiList.delYn eq 'Y' ? 'red' : 'black'};">
                                            ${notiList.boardTitle }
                                        </a>
                                    </td>
                                    <td>${notiList.boardRegDate }</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                    <button type="button" class="btn btn-primary-light btn-sm" onclick="location.href='/admin/noticeInsert'">등록</button>
                </div>
            </div>
